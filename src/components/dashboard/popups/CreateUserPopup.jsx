...
const CreateUserPopup = ({ userInfo, setUserInfo, closeModel,parentId,companyData}) => {
    ...
    const handleSubmit = async () => {
        ...
        if (userInfo?.username === "" || userInfo.email === "" || userInfo.sharedCompanies.length === 0) {
            ...
            try {
                let res = await axios.post(`${config.baseUrl}/account/sub-user/create`, { ...userInfo, parentId })
                if (res.data) {
                    toast.dismiss(loader)
        ...
        <div className="fixed top-0 left-0 w-screen h-screen inset-0 bg-black bg-opacity-50 flex justify-center items-center transition-opacity duration-300 ease-in-out" onClick={() => closeModel(false)}>
            <div className="bg-white rounded-lg shadow-lg p-6 w-[90%] max-w-md transform scale-95 opacity-0 animate-fade-in" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="mb-4">
                    ...
                    <input type="text" id="companyName" name="username" value={userInfo.username} onChange={handleInputChange} placeholder="User Name" className="w-full px-3 py-2 border rounded-md focus:outline-none text-sm" />
                </div>
